...
const ZedOwnQuest = ({
    ...
    const initialValues = {
        type: Enums.ZED_CLAIM,
        text: quest?.text || "Claim $SHELL for owning zed.",
        description: quest?.description ?? "Zed owner can claim $SHELL",
    ...
    const ZedOwnQuestSchema = object().shape({
    ...
    return (
        <Formik
            ...
            validationSchema={ZedOwnQuestSchema}
            validateOnBlur={true}
            ...
            {({ values, errors, status, touched, handleChange, setFieldValue }) => {
                return (
                    <Form>
                        ...
                        <small>Owner of zed can claim $SHELL</small>
                        <div className="row">
                            <QuestFormTemplate
                                values={values}
                                errors={errors}
...
export default withQuestUpsert(ZedOwnQuest);
