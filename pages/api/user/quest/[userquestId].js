    ...
    switch (method) {
        case "GET":
            try {
                ...
                let userQuery = await getWhiteListUserByWallet(whiteListUser.wallet)
                ...
                if (usersOfThisQuest.length !== 0) {
                    let checkOp = usersOfThisQuest.map(uq => {
                        if (uq.user.discordUserDiscriminator !== null && uq.user.discordUserDiscriminator === userQuery.discordUserDiscriminator) {
                            shouldAllowToDoQuest = false;
                        }
                        if (uq.user.twitterUserName !== null && uq.user.twitterUserName === userQuery.twitterUserName) {
                            shouldAllowToDoQuest = false;
                        }
                    })
                    ...
                    if (!shouldAllowToDoQuest) {
                        return res
                            .status(200)
                            .json({ isError: true, message: "Same discord or twitter has been used to do this quest." });
                    }
